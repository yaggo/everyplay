    ...
    if node.nodeType == window.Node.ELEMENT_NODE
      ...
      if value = attr['data-attr']?.value
        for value in value.split ', '
          [attr_name, keypath] = value.split ' '
          attr_val = @resolve keypath
          if attr_val == true
            node.setAttribute attr_name, attr_name # valid value for true boolean attribute
          else if attr_val != false # skip false boolean attribute
            node.setAttribute attr_name, (@resolve keypath)  ? ''
